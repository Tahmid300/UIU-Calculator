...
function calculateCGPA() {
    ...
    retakeCourses.forEach(row => {
        ...
        if (credit > 0 && oldGrade > 0 && newGrade > 0) {
             // For UIU, only the better grade is counted in the total CGPA calculation,
             // and the credits are only counted once in total credits.
             // But here we are using the 'completedCredits' baseline, so we adjust only the points.

            const oldPoints = (credit * oldGrade);
            const newPoints = (credit * newGrade);

            // Subtract the lower grade points and add the higher grade points (or new grade points if it's the latest attempt)
            // A retake is done to replace the OLD grade, so we replace the points regardless of whether the new grade is higher.
            totalPoints = totalPoints - oldPoints + newPoints;
            
            // Retake credits are NOT added to totalCredits because they were already included in 'completedCredits'.
    ...
    // Check if any course data was provided in the current semester
    if (totalCredits === completedCredits && semesterCredits === 0) {
        // Only previous CGPA is present, nothing new to calculate.
        const finalCGPA = currentCGPA;
        ...
        showNotification('সতর্কতা (Warning)', 'এই সেমিস্টারে কোনো কোর্সের ক্রেডিট বা গ্রেড যোগ করা হয়নি। শুধুমাত্র আপনার পূর্বের CGPA দেখানো হচ্ছে।');
        return { success: false };
    }
...
// --- Tuition Fee Calculation Logic (Bug-Fixed) ---
function calculateFee() {
    ...
    if (perCreditFee === 0) {
        showNotification('ইনপুট প্রয়োজন (Input Required)', 'অনুগ্রহ করে প্রতি ক্রেডিট ফি দিন।');
        return { success: false };
    }
    ...
    // Retake has a 50% discount on credit fee in UIU
